# Sakila lab: Read a Product Question in SQL
...
> **This is no longer Chapter 5.** Chapter 5 now runs its investigation on the vendored Noted cohort — see [`../noted/`](../noted/) and `./companion/sql/noted/run_chapter_examples.sh`. This Sakila lab is kept as standalone SQL practice on a larger open-licensed dataset: the joins, grain, and denominator lessons still hold, but the rows and the worked question are not the ones the book prints. Verify it with `./companion/sql/sakila/validate_lab.sh`.
...
To run every concrete SQL example in this lab and compare its output with the tables printed above:
